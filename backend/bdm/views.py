...
class DashboardStatsView(APIView):
    ...
    def get(self, request):
        ...
        active_opportunities = Opportunity.objects.exclude(stage__in=['CLOSED_WON', 'CLOSED_LOST']).count()
        total_revenue = Opportunity.objects.filter(stage='CLOSED_WON').aggregate(total=Sum('expected_revenue'))['total'] or 0
        
        # Pipeline breakdown
        pipeline = Opportunity.objects.values('stage').annotate(count=Count('id')).order_by('stage')
        ...
        upcoming_renewals = Policy.objects.filter(end_date__range=[date.today(), next_30_days]).order_by('end_date')[:5]
